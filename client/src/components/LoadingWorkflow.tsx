...
import { createLoadSession, getOilTypes, getBranches, uploadPhotoToFirebaseStorage } from '@/lib/firebase';
...
export function LoadingWorkflow({ onClose }: LoadingWorkflowProps) {
  ...
  const handleCompleteLoading = async () => {
    try {
      ...
      // Create load session directly in Firestore
      const loadSessionData = {
        oilTypeId: loadingData.oilTypeId,
        oilTypeName: (oilTypes as any[]).find((type: any) => type.id === loadingData.oilTypeId)?.name || 'Unknown Oil Type',
